...
module.exports = class Profanity {
  ...
  getFilter () {
    ...
    const list = blacklist[lang]
    const extraWords = this.config.extraWords
    const exemptWords = this.config.exemptWords
  ...
  async targetIssue (type, issue, comments) {
    const {only} = this.config
    if (only && only !== type) {
      return
    }
    ...
    if (censor) {
      comments = comments || (await this.getIssueComments(issue)).data
  ...
  async censor (type, issue, comments) {
    ...
    if (perform) {
      ...
      if (profane.comments.length > 0) {
        return this.censorComments(profane.comments)
      }
    } else {
  ...
  censorComments (comments) {
    ...
    comments.forEach(comment => {
      ...
      const orderPromise = this.github.issues.editComment({owner, repo, comment_id: id, body})
