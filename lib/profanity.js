  ...
  async markAndSweep (type) {
    ...
    this.getIssues(type).then(res => {
       res.data.items.filter(issue => !issue.locked)
         .forEach(async issue => {
            let profaneComments = false
            let profane = await this.getProfanity(type, issue)
            profaneComments = Object.keys(profane.comments).length
            let isProfane = (profane.title || profane.body || profaneComments)

            if (isProfane && censor){
              this.censor(type, profane)
            }  
            else if(isProfane){  
              this.mark(type, profane)
            }
         })           
    })
