...
module.exports = class Profanity {
  ...
  getProfanity (issue, comments) {
    ...
    const CleanTitle = (this.filter.isProfane(issue.title)) ? this.filter.clean(issue.title) : null

    this.filter.escMarkdown = true
    const CleanBody = (this.filter.isProfane(issue.body)) ? this.filter.clean(issue.body) : null
